...
static char **lege_plicam(const char *iter, int *num_versus)
{
    FILE *f = fopen(iter, "r");
    if (!f) {
        fprintf(stderr, "ERROR: plica '%s' aperiri non potest: %s\n",
                iter, strerror(errno));
        exit(1);
    }
...
static void resolve_ddiff(FILE *fons)
{
    ...
    while (i < num_vs) {
        ...
        if (num_mut >= cap_mut) {
            ...
            mutat_plic = redimensiona(mutat_plic,
...
static char **aedifica_creatum(const char *iter, int *num_versus)
{
    ...
    for (int i = 0; i < num_mut; i++)
        if (strcmp(mutat_plic[i].iter, iter) == 0)
            for (int j = 0; j < mutat_plic[i].num_op; j++)
                if (mutat_plic[i].operationes[j].additum &&
    ...
    for (int i = 0; i < num_mut; i++) {
        if (strcmp(mutat_plic[i].iter, iter) != 0) continue;
        for (int j = 0; j < mutat_plic[i].num_op; j++) {
...
static void scribe_plicam(const char *iter, char **versus, int num_versus)
{
    ...
    FILE *f = fopen(iter, "w");
    if (!f) {
        fprintf(stderr, "ERROR: plica '%s' scribi non potest: %s\n",
                iter, strerror(errno));
        exit(1);
    }
...
{
    fprintf(stderr, "APPLICATIO: %d plicae, %d translationes\n",
            num_plic, num_trans);
    ...
    for (int i = 0; i < num_plic; i++) {
        if (plicae[i].genus == CREATUM)
            argumenta[i] = aedifica_creatum(plicae[i].iter,
                                            &longitudines[i]);
        else if (plicae[i].genus == MUTATUM)
            argumenta[i] = aedifica_mutatum(plicae[i].iter,
                                            &longitudines[i]);
    }
    ...
    for (int i = 0; i < num_plic; i++) {
        if (plicae[i].genus != DELETUM) continue;
        if (remove(plicae[i].iter) == 0) {
            fprintf(stderr, "  D %s\n", plicae[i].iter);
            num_del++;
        } else if (errno != ENOENT) {
            fprintf(stderr, "MONITUM: plica '%s' deleri non potest: %s\n",
